...
var (
	ErrItemrefLoop = errors.New("microdata contains itemref loops")
)
...
func (p *parser) parse() (*Microdata, error) {
	...
	for _, node := range toplevelNodes {
		item := NewItem()
		if err := p.readAttr(item, node, 0); err != nil {
			return nil, err
		}
		if err := p.readItem(item, node, true, 0); err != nil {
			return nil, err
		}
...
// given item.
func (p *parser) readItem(item *Item, node *html.Node, isToplevel bool, level int) error {
	if level > maxRecursionLimit {
		return ErrItemrefLoop
	}
	...
	case hasScope && hasProp:
		subItem := NewItem()
		if err := p.readAttr(subItem, node, level); err != nil {
			return err
		}
		...
		for c := node.FirstChild; c != nil; c = c.NextSibling {
			if err := p.readItem(subItem, c, false, level+1); err != nil {
				return err
			}
		}
		return nil
	...
	for c := node.FirstChild; c != nil; c = c.NextSibling {
		if err := p.readItem(item, c, false, level+1); err != nil {
			return err
		}
	}
	return nil
...
func (p *parser) readAttr(item *Item, node *html.Node, level int) error {
		...
		for _, itemref := range strings.Split(s, " ") {
			if len(itemref) > 0 {
				if n, ok := p.identifiedNodes[itemref]; ok {
					if err := p.readItem(item, n, false, level+1); err != nil {
						return err
					}
				}
			}
